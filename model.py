...
class PositionEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        # create martix of space (seq_len, d_model)
        pe = torch.zeros(seq_len, d_model)
        # Create vector of space (seq_len, 1)
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        # applying the sin for even position
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)

        pe = pe.unsqueeze(0) # (1, seq_len, d_model)

        self.register_buffer('pe', pe) # save as a file
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    @staticmethod # mean you can call this method without having instance of above class
